...
class AADSSOMercer
{
    ...
    public static function print_aad()
    {
        ...
        if ($showAadLogoutMessage) {
            // BEM Block `aad-logout`
            $html .= '<div class="aad-logout">';
            $html .= '<div class="aad-logout__content">';
            $html .= '<h2 class="aad-logout__heading"><strong>Stop!</strong> You are still logged in to Azure AD.</h2>';
            $html .= '<p class="aad-logout__cta-hint">You just logged out of WordPress. If this is a shared computer, you might also want to log out of Azure AD.</p>';
            $html .= '<p class="aad-logout__cta-hint">Log out everywhere you are using your Mercer Office 365 account.</p>';
            $html .= '<a class="aad-logout__cta aad-logout__cta--logout" href="#">';
            $html .= '<span class="aad-login__cta-icon-wrapper aad-login__cta-icon-wrapper--logout">';
            $html .= '<svg class="aad-logout__cta-icon" height="512" width="420.677" viewBox="0 0 0420.677 512"><path d="M420.677 18.286v475.428A18.286 18.286 0 0 1 402.392 512H36.677a18.286 18.286 0 0 1-18.285-18.286v-36.571h36.571v18.286h329.143V36.57H54.963V91.43H18.392V18.286A18.286 18.286 0 0 1 36.677 0h365.715a18.286 18.286 0 0 1 18.285 18.286zm-297.874 316.16l-60.343-60.16h193.646v-36.572H62.46l60.343-60.16-25.966-25.965L5.41 243.017a18.286 18.286 0 0 0 0 25.966l91.428 91.428z"/></svg>';
            $html .= '</span>';  // .aad-login__cta-icon-wrapper
            $html .= '<span class="aad-logout__cta-text">Office 365 SSO Log Out</span>';
            $html .= '</a>';
            $html .= '</div>';  // .aad-logout__content
            $html .= '</div>';  // .aad-logout

            // BEM Block `standard-form-jump`
            $html .= '<div class="standard-form-jump standard-form-jump--after-logout">';
            $html .= '<div class="standard-form-jump__text">';
            $html .= 'Scroll down for standard form';
            $html .= '</div>';  // .standard-form-jump__text
            $html .= '<a class="standard-form-jump__icon-link" href="#standard-login">';
            $html .= '<span class="u-visually-hidden">Jump to standard login form</span>';
            $html .= '<svg class="standard-form-jump__icon" width="511.943" height="512" viewBox="0 0 511.943 512"><path d="M256.335 0c50.601.12 100.978 15.552 142.735 43.77 55.801 37.708 95.9 97.955 108.481 164.653 10.02 53.112 2.776 109.267-20.51 157.938-26.832 56.103-74.865 101.976-132.55 126.079-61.863 25.848-133.88 26.21-196.423.3C100.523 468.9 52.282 423.368 25.133 367.061-3.225 308.238-7.895 238.215 12.7 175.832c17.793-53.896 54.195-101.566 101.67-133.134C155.719 15.199 205.486.196 255.097.001h1.238zm-.755 47.983c-52.15.125-103.385 20.41-141.37 55.773-41.867 38.98-66.558 95.288-66.373 152.984.138 43.471 14.314 86.74 39.862 121.759 30.576 41.912 77.049 71.907 128.514 81.863 51.701 10.002 107.122-.432 151.54-28.83 41.094-26.27 72.55-67.29 86.986-113.921 16.065-51.878 10.865-110.098-14.66-158.452-29.153-55.22-83.848-96.265-146.363-107.75-12.576-2.308-25.37-3.436-38.136-3.425zm.484 280.503l125.289-125.285c2.407-2.072 2.92-2.75 5.752-4.247 7.416-3.92 16.854-3.578 23.977.897 2.712 1.704 3.172 2.42 5.418 4.665.708.962 1.422 1.924 2.13 2.885 1.486 2.808 2.033 3.462 2.862 6.555 1.549 5.795.835 12.122-1.963 17.42-1.497 2.832-2.177 3.347-4.25 5.754L276.073 376.337a22.294 22.294 0 0 1-1.537 2.011c-6.092 7.08-16.434 9.853-25.445 7.141-3.224-.907-6.27-2.473-8.861-4.702a22.645 22.645 0 0 1-3.415-3.69L96.947 237.226c-2.067-2.407-2.752-2.923-4.244-5.754-3.921-7.418-3.581-16.852.898-23.975 1.7-2.712 2.419-3.175 4.664-5.42l2.885-2.13c2.804-1.484 3.46-2.03 6.553-2.86 5.792-1.552 12.12-.837 17.418 1.963 2.833 1.496 3.351 2.175 5.758 4.247z"/></svg>';
            $html .= '</a>';  // .standard-form-jump__icon-link
            $html .= '</div>';  // .standard-form-jump
        }

        // BEM Block `aad-login`
        $html .= '<div class="aad-login">';
        $html .= '<div class="aad-login__content">';
        $html .= '<h2 class="aad-login__heading">Log in to %s</h2>';
        $html .= '<p class="aad-login__cta-hint">Use your Mercer Office 365 account to log in to WordPress</p>';
        $html .= '<a class="aad-login__cta aad-login__cta--login" href="#">';
        $html .= '<span class="aad-login__cta-icon-wrapper aad-login__cta-icon-wrapper--login">';
        $html .= '<svg class="aad-login__cta-icon" height="512" width="402.286" viewBox="0 0 402.286 512"><path d="M402.286 18.286v475.428A18.286 18.286 0 0 1 384 512H18.286A18.286 18.286 0 0 1 0 493.714v-36.571h36.571v18.286h329.143V36.57H36.571V91.43H0V18.286A18.286 18.286 0 0 1 18.286 0H384a18.286 18.286 0 0 1 18.286 18.286zm-268.983 316.16l25.966 25.965 91.428-91.428a18.286 18.286 0 0 0 0-25.966L159.27 151.59l-25.966 25.965 60.343 60.16H0v36.572h193.646z"/></svg>';
        $html .= '</span>';  // .aad-login__cta-icon-wrapper
        $html .= '<span class="aad-login__cta-text">Office 365 SSO Log In</span>';
        $html .= '</a>';
        $html .= '</div>';  // .aad-login__content
        $html .= '</div>';  // .aad-login

        // BEM Block `standard-form-jump`
        $html .= '<div class="standard-form-jump standard-form-jump--after-login">';
        $html .= '<div class="standard-form-jump__text">';
        $html .= 'Scroll down for standard form';
        $html .= '</div>';  // .standard-form-jump__text
        $html .= '<a class="standard-form-jump__icon-link" href="#standard-login">';
